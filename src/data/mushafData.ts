/**
 * Madani Mushaf page data
 * Maps surahs to their starting pages (standard Madani Mushaf - King Fahd Complex)
 */

// Surah start pages (1-indexed, surahId -> page number)
...
/**
 * Get the page image URL for a given page number
 */
...
/**
 * Get which surahs start on or span a given page
 */
export function getSurahsOnPage(page: number): number[] {
  const result: number[] = [];
  for (let surahId = 1; surahId <= 114; surahId++) {
    const startPage = SURAH_START_PAGES[surahId];
    const nextSurahStart = surahId < 114 ? SURAH_START_PAGES[surahId + 1] : TOTAL_PAGES + 1;
    if (startPage <= page && page < nextSurahStart) {
      result.push(surahId);
    }
  }
  return result;
...
/**
 * Get the primary surah on a page (the one that starts latest)
 */
export function getPrimarySurahOnPage(page: number): number {
  const surahs = getSurahsOnPage(page);
  return surahs[surahs.length - 1] || 1;
}
